...
{
    public partial class Frm_FindLine : UIForm, FrmInterface
    {
        ...
        ImageFindLineTool tool;
        public ImageFindLineTool Tool
        {
            get => tool;
            set => tool = value;
        }
        ...
        public void InitCurrImage(HObject image)
        {
        ...
        private void OnResizeDrawingObject(HDrawingObject dobj, HWindow hwin, string type)
        {
            ...
            tool.Row2 = hv_Parms[2].D;
            tool.Column2 = hv_Parms[3].D;
        ...
        private void ParamChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                if (HObjectHelper.ObjectValided(Window.CurrImage))
                {
                    ...
                    //mChangeShowStateDel(true, Tool.CostTime);
                }
                else
        ...
        private void InitParam()
        {
            ...
            uiColorPicker_CheckRegion.Value = tool.mLineShowColor.color;
            uiColorPicker_FindLineColor.Value = tool.mFindLineColor.color;
        ...
        private void FindLine(HObject objFinal)
        {
            ...
            HOperatorSet.AddMetrologyObjectLineMeasure(hv_MetrologyHandle, tool.Row1, tool.Column1, tool.Row2, tool.Column2,//参数导入模型
            ...
            if (resultLine.CountObj() < 1)
            {
                resultLine.Dispose();
                hv_MetrologyHandle.Dispose();
                window.DebugWindow.HalconWindow.DispText("find line failed!", "window", 0, 0, "red", (HTuple)"box", (HTuple)"false");
                ShowToolRunMessage(tool.CostTime.ToString("f2"), false, "find line failed!", true);
                return;
            }
            //显示直线
            window.DebugWindow.HalconWindow.SetColor(tool.mFindLineColor.color.RgbToHex());
            window.DebugWindow.HalconWindow.DispObj(resultLine);
            window.ShowRegionList.Add(new ShowRegionInfoItem() { region = resultLine.Clone(), color = new ColorEx(tool.mFindLineColor.color), colorMode = 1 });

            //显示字符1
            window.DebugWindow.HalconWindow.DispText("find line success!", "window", 0, 0, tool.mShowStringColor.color.RgbToHex(), (HTuple)"box", (HTuple)"false");
            Window.ShowStringInfoList.Add(new ShowStringInfoItem() { showMes = "find line success!", color = tool.mShowStringColor.color, location = new PointEx(0, 0) });
            ...
            tool.LineStartRow = hv_Parameter[0].D;
            tool.LineStartColumn = hv_Parameter[1].D;
            tool.LineEndRow = hv_Parameter[2].D;
            tool.LineEndColumn = hv_Parameter[3].D;
            tool.LineResult = new LineROI();
            tool.LineResult.RowStart1 = hv_Parameter[0].D;
            tool.LineResult.ColumnStart1 = hv_Parameter[1].D;
            tool.LineResult.RowEnd1 = hv_Parameter[2].D;
            tool.LineResult.ColumnEnd1 = hv_Parameter[3].D;
        ...
        private void uiComboTreeView_LineStep_NodeSelected(object sender, TreeNode node)
        {
            ...
            tool.LineSourceToolIDMark = int.Parse(node.Parent.Text.Split('/')[0]);
            tool.LineSourceParam = node.Text;
        }
        ...
        private void MatchToLine()
        {
            ...
            if (tool.ShapeModelSourceToolIDMark > 0)
            {
                ...
                if (hTuple.Length > 0)
                {
                    ...
                    HOperatorSet.AffineTransPoint2d(hTuple, tool.Row2, tool.Column2, out row2Result, out column2Result);
                }
                else
                {
                    row1Result = tool.Row1;
                    column1Result = tool.Column1;
                    row2Result = tool.Row2;
                    column2Result = tool.Column2;
                }
            ...
            HDrawingObject line1 = HDrawingObject.CreateDrawingObject(HDrawingObject.HDrawingObjectType.LINE, row1Result, column1Result, row2Result, column2Result);
            string colo = tool.mLineShowColor.color.RgbToHex();
            line1.SetDrawingObjectParams("color", tool.mLineShowColor.color.RgbToHex());
            AttachDrawObj(line1);
            objList.Add(line1);
